...
class Version:
    ...
    def build_docker_image(self, force):
        ...
        if force == True or len(client.images.list(image_name)) == 0:
            ...
            target_path = curr_dir + '/../systems/' + self.application.abbr + '/' + self.version_number + '/build-image'
            print(target_path)
    ...
    def build_compile_docker_image(self, force):
        ...
        if force == True or len(client.images.list(image_name)) == 0:
            curr_dir = os.path.dirname(os.path.abspath(__file__))
            target_path = curr_dir + '/../systems/' + self.application.abbr + '/' + self.version_number + '/compile-src'
            # build the image
            client.images.build(path=target_path, tag=image_name)
        pass
    ...
    def start_cluster(self, config, test_dir, upgrade_edge, test_name, test_id):
        ...
        yml_file = self.generate_yml_wrapper(config, test_dir, subnet, test_name, network_name)
    ...
    def teardown_cluster(self, yml_file, test_dir, docker_console_log, config):
        ...
        p = subprocess.Popen(['docker-compose', '-f', yml_file, 'logs'], stdout=file_output, cwd=test_dir)
        p.wait()
        file_output.close()
