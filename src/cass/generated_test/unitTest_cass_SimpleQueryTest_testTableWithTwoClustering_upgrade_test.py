...
testId = 4
testName = 'SimpleQueryTest_testTableWithTwoClustering'
...
class casstestTableWithTwoClusteringUpgradeTest(upgrade_test_base.UpgradeTestBase):
    ...
    def test_old_version(self):
        ...
        test.tester.setUpClass(num_nodes, subnet)
        test.tester.beforeTest()
        test.testTableWithTwoClustering()
        createdTables.extend(test.tester.createdTables)
if (__name__ == '__main__'):
    old_version = cass_upgrade_path.CassVersion('cassandra', 'cassandra-2.1.0')
    new_version = cass_upgrade_path.CassVersion('cassandra', 'cassandra-2.2')
    config = Config(10, 1)
    application_start_interval = 120
    upgrade_edge = cass_upgrade_path.CassUpgradeEdge(old_version, new_version)
    test = casstestTableWithTwoClusteringUpgradeTest(upgrade_edge, config, application_start_interval, 'unittest', testName, str(testId))
    test.test_upgrade()
