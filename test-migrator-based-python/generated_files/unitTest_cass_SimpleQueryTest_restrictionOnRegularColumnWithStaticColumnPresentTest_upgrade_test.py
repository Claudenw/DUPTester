...
createdTables = []
import CQLTester
import SimpleQueryTest
testId = 18
testName = 'SimpleQueryTest_restrictionOnRegularColumnWithStaticColumnPresentTest'

class cassrestrictionOnRegularColumnWithStaticColumnPresentTestUpgradeTest(upgrade_test_base.UpgradeTestBase):

    def test_new_version(self, test_node_id):
        num_nodes = self.config.num_nodes
        subnet = ((utils.app_name_2_subnet(old_version.application.name) + str(testId)) + '.')
        test = SimpleQueryTest.SimpleQueryTest()
    ...
    def test_old_version(self):
        ...
        test.tester.setUpClass(num_nodes, subnet)
        test.tester.beforeTest()
        test.restrictionOnRegularColumnWithStaticColumnPresentTest()
...
if (__name__ == '__main__'):
    ...
    test = cassrestrictionOnRegularColumnWithStaticColumnPresentTestUpgradeTest(upgrade_edge, config, application_start_interval, 'unittest', testName, str(testId))
    test.test_upgrade()
